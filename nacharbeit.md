...
| **C17** | `libreverbum/pipeline.py`, `run_chapter` | Der buchweite Eigennamenanteil (`291a19c`, 26.08.2026) verlangt einen vollen spaCy-Lauf über **alle** Kapitel des Buchs, bei **jedem** Kapiteldurchlauf erneut: gemessen 22 s (Dorian Gray) beziehungsweise 29 s (Sherlock) gegenüber vorher rund 1,1 s für `run_chapter`. Regel 14 verlangt für einen Zwischenspeicher einen gemessenen Anlass — der liegt damit vor. Naheliegend ist, den Wert je Buch im Profil zu halten (`book`-Tabelle), statt ihn bei jedem Lauf neu zu berechnen; zu entscheiden ist, wann er verfällt (neue spaCy-Fassung, geänderte Datei) |
