...
| Davon (`unknown_lemma_count`/`unique_unknown_lemma_count`), wie viele **ohne** Wörterbucheintrag sind — `entry.candidates` besteht nur aus dem `uncertain`-Platzhalter, keine einzige echte Wörterbuchzeile (Befund D1, Nachbesserung Durchsicht 79b4479, 24.09.2026: „Wörter zu lernen" zählte diese Grundformen bisher ungekennzeichnet mit, obwohl ein Teil davon kein Lernstoff ist). Je Kapitel `Coverage.unknown_lemma_count_without_dictionary_entry`/`ChapterDifficulty.unknown_lemma_count_without_dictionary_entry`, fürs Buch `BookDifficulty.unique_unknown_lemma_count_without_dictionary_entry` (Vereinigung wie `unique_unknown_lemma_count`, nicht Summe) | `unknown_lemma_count_without_dictionary_entry` / `unique_unknown_lemma_count_without_dictionary_entry` |
